...
# Adding MCP Servers to CastCodes
...
Config file paths:
- **Global (user-scoped):** `~/.cast-codes/.mcp.json`
- **Project-scoped:** `{repo_root}/.cast-codes/.mcp.json`
...
- Project-scoped configs (`{repo_root}/.cast-codes/.mcp.json`) run from the repo root.
- Global configs (`~/.cast-codes/.mcp.json`, `~/.claude.json`, etc.) run from the home directory.
...
- CastCodes auto-detects changes to `.mcp.json` files on save — no restart required.
- Configured servers appear in CastCodes Settings under MCP, labeled **"Detected from CastCodes"**.
