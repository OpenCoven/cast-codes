...
fn test_build_worktree_toml_quotes_base_branch() {
    ...
    let cases = [
        (Some("my-feature"), "my-feature"),
        (None, "autogenerated-feature"),
    ];

    for (worktree_branch_name, rendered_branch_name) in cases {
        let toml_str = build_worktree_config_toml(
            "Worktree: my-project",
            "/Users/me/repo",
            base_branch,
        ...
        if let Some(name) = worktree_branch_name {
            params.insert("worktree_branch_name".to_string(), name.to_string());
        }
        let (_, template) = render_tab_config(
            &config,
            &params,
            worktree_branch_name
                .is_none()
                .then_some(rendered_branch_name),
        );
        ...
        if let PaneTemplateType::PaneTemplate { commands, .. } = template {
            ...
            assert_eq!(
                commands[0].exec,
                format!(
                    "git worktree add -b {rendered_branch_name} {} {}",
                    generated_worktree_path_string("/Users/me/repo", rendered_branch_name),
