...
static DEFAULT_TIPS: LazyLock<Vec<AgentTip>> = LazyLock::new(|| {
    vec![
        AgentTip {
            ...
            link: Some(USER_DOCS_URL.to_string()),
            binding_name: None,
            action: None,
        ...
        AgentTip {
            description: "Enable shell integration for a remote SSH session to improve terminal integration in that environment.".to_string(),
            link: Some(USER_DOCS_URL.to_string()),
            binding_name: None,
            action: None,
            kind: AgentTipKind::General,
