                ...
                if is_open_for_same_conversation {
                    ...
                    let rename_items: Vec<MenuItem<ConversationListViewAction>> =
                        match conversation_id {
                            ConversationOrTaskId::ConversationId(ai_conversation_id) => {
                                let history_model = BlocklistAIHistoryModel::as_ref(ctx);
                                ...
                                if is_shared_viewer {
                                    ...
                                    // Compute `has_user_set_title` inside the same scope as `item`
                                    // so the `&ConversationNavigationData` borrow doesn't outlive
                                    // the owned `ConversationOrTask` that backs it.
                                    let has_user_set_title = self
                                        .view_model
                                        .as_ref(ctx)
                                        .get_item_by_id(&conversation_id, ctx)
                                        .map(|item| {
                                            item.navigation_data()
                                                .map(|nav| nav.has_user_set_title)
                                                .unwrap_or(false)
                                        })
                                        .unwrap_or(false);
                                    let mut entries = vec![MenuItemFields::new("Rename conversation")
                                        .with_on_select_action(
                            ...
                            ConversationOrTaskId::TaskId(_) => Vec::new(),
                        };
                            ...
                            match list_item {
                                ...
                                ListItem::Conversation(entry) => {
                                    ...
                                    let is_renaming = match entry.id {
                                        ConversationOrTaskId::ConversationId(id) => {
                                            renaming_conversation_id == Some(id)
                                        }
                                        ConversationOrTaskId::TaskId(_) => false,
                                    };
                                    Some(render_item(
                                        ItemProps {
