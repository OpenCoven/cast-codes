...
pub fn render_item(props: ItemProps<'_>, app: &AppContext) -> Box<dyn Element> {
    ...
    let title_slot: Box<dyn Element> = if is_renaming {
        ...
        let mut title_text =
            Text::new_inline(conversation.title(app), font_family, title_font_size)
                .with_color(theme.main_text_color(theme.background()).into());
    ...
    let allow_rename = matches!(conversation_id, ConversationOrTaskId::ConversationId(_));
    ...
    let hoverable_element = if open_action.is_some() && !is_renaming {
        hoverable
            .with_cursor(Cursor::PointingHand)
