...
/// Per-pane minimum-width lookup used by the splitter clamp. The caller maps a
/// leaf [`PaneId`] to the minimum horizontal pixel width below which the pane
/// becomes illegible (see [`crate::pane_group::pane::PaneContent::min_pane_width`]).
/// Unknown ids fall back to [`crate::pane_group::MINIMUM_PANE_SIZE`].
...
impl PaneNode {
    ...
    fn max_leaf_min_pane_width(&self, pane_min_width: PaneMinWidthFn) -> f32 {
        match self {
            ...
            .position(|divider| divider.id == border_id)
        {
            ...
            let (min_1, min_2) = match self.axis {
                SplitDirection::Horizontal => (
                    ...
                    base_min.max(self.nodes[idx + 1].1.max_leaf_min_pane_width(pane_min_width)),
                ),
                SplitDirection::Vertical => (base_min, base_min),
            };

            // Omit noise in dragging.
            if size_1 + delta < min_1
                || size_2 - delta < min_2
